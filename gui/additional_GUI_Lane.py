...
class AdditionalGUILane:
    ...
    def showViolationFrame(self):
        ...
        self.violation_message_type.config(text='Type: Lane Cross Violation', width='300')
        ...
        self.violaton = tk.Message(self.recent_violation_frame)
        self.violaton.config(anchor='w', background='#ffff80', cursor='arrow', font='{@Yu Gothic UI Light} 12 {}')
        self.violaton.config(relief='groove', text='Description:  A vehicle at (200,300,400,500) commenced a left lane'
                                                   'cross violation. Saving the violation video.', width='290')
        self.violaton.place(anchor='nw', relheight='0.65', relwidth='1.0', relx='0.0', rely='0.38', x='0', y='0')
        self.recent_violation_frame.config(height='300', relief='flat', takefocus=False, text='Recent Violation')
        self.recent_violation_frame.config(width='300')
        self.recent_violation_frame.grid(row=2, column=1)
